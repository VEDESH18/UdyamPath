...
import { Eye, EyeOff, Loader2, Chrome, ArrowRight, Sprout } from 'lucide-react';

export default function Login() {
  const navigate = useNavigate();
  const { signInWithEmail, signInWithGoogle, authError, setAuthError } = useAppContext();
    ...
    <div className="min-h-screen bg-[#0f0f0f] flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md animate-slide-up relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-poppins font-black text-white mb-2">Welcome back! 👋</h1>
          <p className="text-white/50 text-sm">Your startup journey continues here.</p>
        </div>
        ...
        <div className="bg-[#1a1a1a] rounded-3xl border border-white/8 p-8 shadow-[0_25px_60px_rgba(0,0,0,0.5)]">
          ...
          {error && (
            <div className="mb-4 p-3 rounded-xl bg-red/10 border border-red/20 text-red text-sm font-medium text-center">
              {error}
            </div>
          )}
        ...
        <p className="text-center text-white/40 text-sm mt-6">
          New to UdyamPath?{' '}
          <Link to="/signup" className="text-[#58CC02] font-bold hover:underline">
